...
- **`public/vendor/cubejs.js`** — the `cubejs` cube model/move engine,
  vendored as a plain script (sets `window.Cube`).
- **`public/js/solver.js`** — a real beginner layer-by-layer solver (browser
  ...
  1000`).
- **`public/js/cube3d.js`** — a real 3D Rubik's cube (Three.js): 26 fixed-slot
...
- **`public/app.js`** — on load, generates a random scramble, solves it, and
...
`.github/workflows/deploy.yml` publishes `public/` on push to `main`
